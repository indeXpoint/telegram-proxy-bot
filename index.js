...
function getTicket(userId, botKey) {
  if (!tickets[userId]) {
    tickets[userId] = {
      ticketId: crypto.randomBytes(3).toString("hex"),
      status: "open",
      lastBotKey: botKey,
    };
  } else {
    ...
    if (update.callback_query) {
      ...
      if (data.startsWith("close_")) {
        ...
        await fetch(`${api(token)}/answerCallbackQuery`, {
          ...
          body: JSON.stringify({
            callback_query_id: callback.id,
            text: `Ticket #${tickets[userId].ticketId} closed ✅`,
          }),
        });
        ...
        await fetch(`${api(token)}/answerCallbackQuery`, {
          ...
          body: JSON.stringify({
            callback_query_id: callback.id,
            text: `Send your reply to ticket #${tickets[userId].ticketId}`,
          }),
        });
      }
...
We’ll reply here as soon as possible.`;
    ...
    if (fromId === ADMIN_ID) {
      ...
      const replyingUserId = Object.keys(replyQueue).find(
        (u) => replyQueue[u] === botKey,
      );
      if (replyingUserId) {
    ...
    if (method === "sendPhoto")
      options.photo = msg.photo[msg.photo.length - 1].file_id;
...
app.listen(PORT, () =>
  console.log(`✅ Multi-bot support relay running on port ${PORT}`),
);
